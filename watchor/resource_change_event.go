...
func (c *ResourceChangeWatchClient) init(params *ResourceChangeWatchStartParams) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *ResourceChangeWatchClient) poll() {
	...
	for {
		...
		if err := c.pollOnce(); err != nil {
			// exponential backoff = 0.5 * 2^i seconds
			time.Sleep(time.Duration(math.Pow(2, float64(requestErrCount))*0.5) * time.Second)
			requestErrCount++
