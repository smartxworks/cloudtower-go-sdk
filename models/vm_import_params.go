...
type VMImportParams struct {
	...
	MarshalOpts *VMImportParamsMarshalOpts `json:"-"`
}

type VMImportParamsMarshalOpts struct {
	...
	OwnerID_Explicit_Null_When_Empty bool
...
func (m VMImportParams) MarshalJSON() ([]byte, error) {
	var b bytes.Buffer
	b.WriteString("{")
	...
	// handle nullable field owner_id
	if m.OwnerID != nil {
		if !first {
			b.WriteString(",")
		}
		b.WriteString("\"owner_id\":")
		bytes, err := swag.WriteJSON(m.OwnerID)
		if err != nil {
			return nil, err
		}
		b.Write(bytes)
		first = false
	} else if m.MarshalOpts != nil && m.MarshalOpts.OwnerID_Explicit_Null_When_Empty {
		if !first {
			b.WriteString(",")
		}
		b.WriteString("\"owner_id\":null")
		first = false
	}
