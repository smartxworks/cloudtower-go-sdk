...
type ClientService interface {
	...
	UpdateNfsInode(params *UpdateNfsInodeParams, opts ...ClientOption) (*UpdateNfsInodeOK, error)
...
/*
  UpdateNfsInode update nfs inode API
*/
func (a *Client) UpdateNfsInode(params *UpdateNfsInodeParams, opts ...ClientOption) (*UpdateNfsInodeOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewUpdateNfsInodeParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "UpdateNfsInode",
		Method:             "POST",
		PathPattern:        "/update-nfs-inode",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &UpdateNfsInodeReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*UpdateNfsInodeOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for UpdateNfsInode: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
