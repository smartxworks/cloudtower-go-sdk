...
func (a *Client) GetWitnessServices(params *GetWitnessServicesParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetWitnessServicesOK, error) {
	...
	op := &runtime.ClientOperation{
		ID:                 "GetWitnessServices",
		Method:             "POST",
		PathPattern:        "/get-witness-services",
	...
	success, ok := result.(*GetWitnessServicesOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for GetWitnessServices: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
