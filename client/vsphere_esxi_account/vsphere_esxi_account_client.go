...
func (a *Client) UpdateVsphereEsxiAccount(params *UpdateVsphereEsxiAccountParams, opts ...ClientOption) (*UpdateVsphereEsxiAccountOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewUpdateVsphereEsxiAccountParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "UpdateVsphereEsxiAccount",
		Method:             "POST",
		PathPattern:        "/update-vsphere-esxi-account",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &UpdateVsphereEsxiAccountReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*UpdateVsphereEsxiAccountOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for UpdateVsphereEsxiAccount: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
